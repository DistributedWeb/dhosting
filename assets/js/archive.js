...
$(function () {
  ...
  var removeForm = $('#remove-vault-form')
  ...
  $('#show-remove-vault-form').on('click', function () {
    removeForm.addClass('open')
  })

  $('#cancel-remove-vault').on('click', function (e) {
    e.preventDefault()
  ...
  renameFormInput.on('keyup', function (e) {
    ...
    var url = 'dweb://' + (name ? (name + '.' + params.hostname) : params.key)
  ...
  renameForm.on('submit', function (e) {
    ...
    var xhr = $.post('/v2/vaults/item/' + params.key, values)
  ...
  removeForm.on('submit', function (e) {
    ...
    var xhr = $.post('/v2/vaults/remove', values)
    xhr.done(function (res) {
      // success, redirect
