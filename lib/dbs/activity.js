...
class ActivityDB {
  constructor (dhost) {
    // create levels
    this.globalActivityDB = sublevel(dhost.db, 'global-activity', { valueEncoding: 'json' })
    this.metaDB = sublevel(dhost.db, 'global-activity-meta', { valueEncoding: 'json' })
    this.usersIndexDB = sublevel(dhost.db, 'global-activity-users-index')

    // promisify
    levelPromise.install(this.metaDB)
  ...
  listUserEvents (username, opts = {}) {
    return new Promise((resolve, reject) => {
      ...
      var stream = this.usersIndexDB.createReadStream(opts)
        .pipe(dws2.obj(function (entry, enc, cb) {
          // load the record
