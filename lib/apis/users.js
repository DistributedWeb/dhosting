...
const {UnauthorizedError, ForbiddenError, NotFoundError, BadRequestError, COHORT_STATE_REGISTERED} = require('../const')
...
module.exports = class UsersAPI {
  constructor (dhost) {
    ...
    this.sessions = dhost.sessions
    this.proofs = dhost.proofs
    this.mailer = dhost.mailer
    this.stripe = dhost.config.stripe ? Stripe(dhost.config.stripe.secretKey) : null
  }
  ...
  async doRegister (req, res) {
    ...
    req.sanitizeBody('username').toLowerCase()
    ...
    try {
      ...
      // log analytics
      /* dont await */ req.logAnalytics('register')
      /* dont await */ this.usersDB.updateCohort(record, COHORT_STATE_REGISTERED)
    } finally {
      release()
    }
    ...
    this.mailer.send('verification', {
      ...
      username,
      emailVerificationNonce,
      emailVerificationLink: `http://${this.config.hostname}/v2/accounts/verify?${qs}`
    })
    // log the verification link
    if (this.config.env === 'development') {
      console.log('Verify link for', username)
      console.log(`http://${this.config.hostname}/v2/accounts/verify?${qs}`)
    }
  ...
  async updateAccount (req, res) {
    ...
    // validate & sanitize input
    req.checkBody('profileURL').isDWebURL()
    ;(await req.getValidationResult()).throw()
    req.sanitizeBody('profileURL').toDWebDomain()
    ...
    try {
      ...
      if (profileURL && profileURL !== userRecord.profileURL) {
        // remove old profile-dpack from flock
        // TODO

        // add new profile-dpack to flock
        // TODO

        // generate a new proof & update record
        userRecord.profileVerifyToken = this.proofs.generate(userRecord)
        userRecord.isProfileDPackVerified = false
  ...
  async upgradePlan (req, res) {
    ...
    if (!this.stripe) throw new ForbiddenError('This service does not accept payments.')
    if (sessionUser.stripeSubscriptionId) {
      console.error('Session user already has a plan in upgradePlan', sessionUser)
  ...
  async doForgotPassword (req, res) {
    ...
    this.mailer.send('forgot-password', {
      ...
      username: userRecord.username,
      forgotPasswordNonce,
      forgotPasswordLink: `http://${this.config.hostname}/reset-password?${qs}`
    })

    // log the verification link
    if (this.config.env === 'development') {
      console.log('Forgot-password link for', userRecord.username)
      console.log(`http://${this.config.hostname}/reset-password?${qs}`)
    }
  }
  ...
  async get (req, res) {
    ...
    switch (req.query.view) {
      case 'vaults':
        await Promise.all(userRecord.vaults.map(async (vault) => {
          var manifest = await this.vaultr.getManifest(vault.key)
          if (manifest) {
            vault.title = manifest.title
            vault.description = manifest.description
          } else {
            vault.title = ''
            vault.description = ''
          }
  ...
  async _createProPlan (token, userRecord) {
    ...
    try {
      ...
      try {
        ...
        console.error('Upgrade failed due to invalid payment token', userRecord, e)
        let err = new Error()
        err.status = 422
      ...
      this.usersDB.update(userRecord.id, {
        ...
        namedVaultQuota: this.config.proNamedVaultsLimit,
        stripeCustomerId: customerId,
