...
const {DWEB_KEY_REGEX, COHORT_STATE_ACTIVE, NotFoundError, UnauthorizedError, ForbiddenError} = require('../const')
...
module.exports = class VaultsAPI {
  ...
  async getByName (req, res) {
    ...
    // lookup vault
    const findFn = (DWEB_KEY_REGEX.test(vaultname))
      ? a => a.key === vaultname
      : a => a.name === vaultname
  ...
  async _getVaultInfo (userRecord, vault) {
    ...
    if (vault.name) {
      ...
      additionalUrls = [`dweb://${niceUrl}`, `http://${niceUrl}`]
    }
