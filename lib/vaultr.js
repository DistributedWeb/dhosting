...
const {du, dwebStat, dwebReadFile, dwebIterateFiles, isObject} = require('./helpers')
...
module.exports = class Vaultr extends EventEmitter {
  ...
  async getManifest (key) {
    ...
    try {
      var st = await dwebStat(vault, '/dweb.json', {cached: true})
      if (vault.manifest) {
      ...
      vault.manifest = JSON.parse(await dwebReadFile(vault, '/dweb.json', {cached: true}))
  ...
  async _computeVaultLatestStats (vault) {
    ...
    try {
      ...
      await dwebIterateFiles(co, '/', (fileSt) => {
