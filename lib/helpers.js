...
async function dwebIterateFiles (vault, filepath, fn) {
  var names = await dwebReaddir(vault, filepath, {cached: true})
  for (let name of names) {
    let filepath2 = path.join(filepath, name)
    let st = await dwebStat(vault, filepath2, {cached: true})
    if (st.isDirectory()) {
      await dwebIterateFiles(vault, filepath2, fn)
    } else {
      fn(st)
    }
  }
}
exports.dwebIterateFiles = dwebIterateFiles

function dwebStat (vault, filepath, opts = {}) {
...
exports.dwebStat = dwebStat

function dwebReaddir (vault, filepath, opts = {}) {
  return new Promise((resolve, reject) => {
...
exports.dwebReaddir = dwebReaddir

function dwebReadFile (vault, filepath, opts = {}) {
  return new Promise((resolve, reject) => {
...
exports.dwebReadFile = dwebReadFile
